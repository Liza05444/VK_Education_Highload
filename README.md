...
2. Сервер проверяет, существует ли `since_id` в `inbox_events`:
    * Если да (клиент офлайн <= 7 дней) — сервер выбирает из `inbox_events` все строки с `id > since_id` для данного `user_id`, затем по полученным парам `(chat_id, message_id)` запрашивает тела сообщений из ScyllaDB и возвращает клиенту ответ.
