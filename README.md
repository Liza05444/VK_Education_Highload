...
3. Для Kubernetes в DC5 требуется 88 CPU requests. С учетом 20% накладных расходов Kubernetes: 88 × 1,2 ≈ 106 CPU. Минимум: 106 / 16 ≈ 7 серверов. Для DC5 принято 12 серверов с запасом нагрузки; для остальных ДЦ количество снижено пропорционально доле трафика: 12 + 6 + 6 + 4 + 4 = 32 сервера.
4. Kafka размещается на 17 серверах: 5 брокеров в DC5 и по 3 брокера в остальных 4 ДЦ. Для DC5 выбрано 5 брокеров вместо минимальных 3, чтобы при отказе одного брокера не перегружать оставшиеся узлы.
