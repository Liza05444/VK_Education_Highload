...
## 7.1 Паттерн Transactional Outbox

Для обеспечения согласованности применяется паттерн Transactional Outbox:

1. Любое изменение данных в PostgreSQL записывается в рамках одной транзакции вместе с событием в таблицу `outbox_events`. Благодаря WAL, PostgreSQL гарантирует, что если транзакция зафиксирована, событие надежно сохранено на диске.

2. Сервис-релей доставляет это событие в очередь Kafka.

3. Воркеры доставляют изменения во все связанные компоненты, обеспечивая согласованность данных в распределенной среде.

## 7.2 Алгоритм отправки сообщения

### 1. Стратегии обновления данных в БД

* Запись сообщения: тело сообщения сохраняется в ScyllaDB. Сразу после успешного ответа от ScyllaDB, в PostgreSQL открывается транзакция, которая создает запись в `outbox_events`. В случае сбоя клиент инициирует повтор, а ScyllaDB перезаписывает данные по тому же ID.

* **Личные чаты и группы:** воркеры из очереди обновляют поля `last_message_id` и `last_message_preview` в таблице `user_dialogues` для всех участников. Это позволяет отрисовать список чатов одним запросом.

* **Каналы и супергруппы:** при превышении лимита в 200 участников группа автоматически трансформируется в супергруппу. Для таких чатов массовое обновление строк в `user_dialogues` не производится. Актуальное состояние хранится в одной строке таблицы `chats`, откуда клиент подтягивает данные при чтении.

### 2. Push-уведомление

Далее генерируется Push-уведомление, содержащее только `chat_id`. Текст сообщения отсутствует в целях приватности и минимизации трафика. Получив ID чата, клиентское приложение понимает, какой диалог переместить в топ, и инициирует фоновую подгрузку дельты данных.

## 7.3 Механизм синхронизации состояния

### 1. Подгрузка дельты

Клиент отправляет серверу свой `last_sync_message_id`. Сервер сравнивает его с актуальным состоянием в базе и отдает только дельту — список новых сообщений.

### 2. Синхронизация через Redis

* **Запись:** при прочтении чата на телефоне сервер обновляет `last_sync_message_id` для текущей сессии. Также через `user_id` обновляются данные для других сессий пользователя.

* **Синхронизация:** остальные устройства пользователя узнают об обновлении при помощи Long Polling. Сервер отдает актуальный `last_sync_message_id`.

Т.о., о новом сообщении получатель узнаёт через push или канал обновлений (Long Polling).
