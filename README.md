...
* **PostgreSQL Table `user_dialogues`:** Денормализованный список чатов пользователя с превью и счётчиком непрочитанных. Обновляется атомарно вместе с `inbox_events`.
...
Выбранный вариант **API → Kafka → Inbox** разделяет ответственность между компонентами:
