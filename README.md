...
Целевая конфигурация — серверы с 4 CPU и сетевыми интерфейсами 100GbE.
...
| --- | --- | --- | --- |
| **L4** | 10 | CPU 4 Cores, NIC 100GbE | N × 2 |
...
| Характер нагрузки | Норма на 1 ядро |
| :--- | :--- |
| **Auth API** (JWT, Redis) | 100 RPS |
| **Core API** (Бизнес-логика, PG) | 10 RPS |
| **Message API** (Архив сообщений, Scylla) | 100 RPS |
| **Media API** (S3 Proxy) | 100 RPS |
| **Inbox worker** | 100 событий/с |
| **Archive worker** | 100 записей/с |
| **Push API** | 100 RPS |

### Сводная таблица ресурсов
| Сервис | Нагрузка (RPS) | Расчет | CPU (ядер) | RAM |
| :--- | :--- | :--- | :--- | :--- |
| **auth-service** | 258 334 | 258 334 / 100 = 2 584 → 324 × 8 | 2 592 | 2 592 ГБ |
| **core-service** | 43 750 | 43 750 / 10 = 4 375 → 548 × 8 | 4 384 | 8 768 ГБ |
| **message-service** | 125 000 | 125 000 / 100 = 1 250 → 160 × 8 | 1 280 | 2 560 ГБ |
| **media-service** | 45 834 | 45 834 / 100 = 458,3 → 60 × 8 | 480 | 960 ГБ |
| **inbox-worker** | 250 000 | 250 000 / 100 = 2 500 → 316 × 8 | 2 528 | 2 528 ГБ |
| **archive-worker** | 125 000 | 125 000 / 100 = 1 250 → 160 × 8 | 1 280 | 1 280 ГБ |
| **notification-service** | 50 000 | 50 000 / 100 = 500 → 64 × 8 | 512 | 512 ГБ |
| **Итого** | | | **13 056** | **~19,2 ТБ** |
...
| Тип сервера | Модель | Конфигурация | Cores | Количество | Цена | Стоимость | Амортизация(на 5 лет)/мес | Аренда/мес |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Kubernetes Cluster** | CyberServe Xeon SP1-102G | Xeon Silver 4514Y / 128GB RAM / 2x1TB NVMe / 10Gbps | 16 | 2 452 | €5 490 | €13 461 480 | €224 358 | AX102: €122.30 × 2 452 = €299 879.60 |
...
| :--- | :--- | :--- | :--- | :--- |
| **auth-service** | 324 | 8 | 8 ГБ | Авторизация и сессии |
| **core-service** | 548 | 8 | 16 ГБ | Основная бизнес-логика |
| **message-service** | 160 | 8 | 16 ГБ | Доступ к архиву сообщений |
| **media-service** | 60 | 8 | 16 ГБ | Проксирование в Ceph |
| **inbox-worker** | 316 | 8 | 8 ГБ | Kafka, inbox_events |
| **archive-worker** | 160 | 8 | 8 ГБ | Kafka, ScyllaDB архив |
| **notification-service** | 64 | 8 | 8 ГБ | push уведомления |
...
1. Количество серверов в DC1-DC4 ниже, чем в DC5; их количество пропорционально доле трафика (п. 3.3).

Примечание:
1. Для Kubernetes в DC5 требуется 13 056 CPU. С учетом 20% накладных расходов Kubernetes: 13 056 × 1,2 ≈ 15 668 CPU. Минимум: 15 668 / 16 ≈ 980 серверов. Для DC5 принято 980 серверов; для остальных ДЦ количество снижено пропорционально доле трафика: 980 + 429 + 429 + 307 + 307 = 2 452 сервера.
2. Kafka размещается на 17 серверах: 5 брокеров в DC5 и по 3 брокера в остальных 4 ДЦ. Для DC5 выбрано 5 брокеров вместо минимальных 3, чтобы при отказе одного брокера не перегружать оставшиеся узлы.
