...
| **`static.telegram.org`** | Раздача статических ресурсов (JS/CSS) |
| **`telegram.org`** | Основной сайт (FAQ, документация, дистрибутивы) |

## 3.2 Расположение дата-центров

Для обеспечения RTT в пределах 50 мс в ключевых регионах присутствия [4] используется система из 5 основных ДЦ:
...
* **Latency-based DNS:** На основе Geo-IP базы пользователю возвращается IP ближайшего регионального ДЦ с минимальным RTT. Устанавливается короткий **TTL (60 сек)** для быстрой смены ДЦ в случае аварии.
* **BGP Anycast:** Применяется для `api.telegram.org`. Несколько ДЦ анонсируют один и тот же IP-адрес. Сетевые роутеры направляют пакеты по кратчайшему пути (наименьшее число хопов). Применение BGP Anycast ограничено доменом api.telegram.org, т.к. для него приоритетами являются минимальная задержка и быстрая установка соединения. Для домена media.telegram.org использование Anycast нецелесообразно из-за риска разрыва длительных TCP-сессий при изменении BGP-маршрутов.

## 3.5 Механизмы регулировки трафика

1. **Weighted Round-Robin:** Использование весовых коэффициентов в GSLB для управления долями входящего трафика (например, для DC1 и DC3, для DC2 и DC4).
