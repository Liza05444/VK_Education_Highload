...
Все сервисы развёртываются в 5 дата-центрах (DC1–DC5) в собственном колокейшн-размещении. Stateless-сервисы (Go-микросервисы) работают под управлением Kubernetes; stateful-компоненты (PostgreSQL, ScyllaDB, Redis, Kafka, Ceph) — на выделенных физических серверах вне оркестрации.

Расчёты выполнены для **DC5 (Азия, Сингапур)** — крупнейшего ДЦ с долей **40% мирового трафика** и пиковой нагрузкой **258 334 RPS / 410 Гбит/с**. Для остальных ДЦ количество серверов пропорционально их доле трафика.

## 11.1 Ресурсные требования сервисов (DC5, пиковая нагрузка)

| Технология и характер нагрузки | Норма |
| --- | --- |
| Go — лёгкое JSON API (запись в Kafka, Redis-lookup) | 5 000 RPS / ядро |
| Go — средняя бизнес-логика (запросы к PostgreSQL) | 2 000 RPS / ядро |
| Go — WebSocket-соединения | 50 000 соед. / ядро |
| Nginx SSL termination | 500 CPS / ядро |

| Сервис | Пиковая нагрузка (DC5) | CPU (ядер) | RAM |
| --- | --- | --- | --- |
| **LVS (L4)** | 410 Гбит/с | 80 | 80 ГБ |
| **Nginx (L7)** | 258 334 CPS / 410 Гбит/с | 640 | 640 ГБ |
| **message-service** | 125 000 RPS | 32 | 32 ГБ |
| **sync-service** | ~5 млн WebSocket-соед. | 96 | 192 ГБ |
| **media-service** | 45 834 RPS / 410 Гбит/с | 192 | 192 ГБ |
| **auth-service** | 258 334 RPS | 32 | 8 ГБ |
| **chat-service** | 43 750 RPS | 24 | 48 ГБ |
| **inbox-worker** | 250 000 fan-out/с | 80 | 40 ГБ |
| **archive-worker** | 125 000 write/с в ScyllaDB | 16 | 4 ГБ |
| **notification-service** | ~50 000 push/с | 8 | 4 ГБ |
| **PostgreSQL** | 375 K write + 500 K read QPS | 192 | 768 ГБ |
| **ScyllaDB** | 125 K write + 417 K read ops/с | 128 | 512 ГБ |
| **Redis** | ~750 K ops/с | 48 | 192 ГБ |
| **Kafka** | 125 000 сообщ./с | 48 | 192 ГБ |
| **Ceph RGW** | 410 Гбит/с | 160 | 320 ГБ |
| **Ceph OSD** | 1+ ЭБ хранилище | 80 | 160 ГБ |

**sync-service.** Из раздела 2.1: пользователь проводит в приложении 41 мин/день. Конкурентные соединения в DC5: 180 млн × 41 / (24 × 60) ≈ 5,1 млн. При 50 000 соед./ядро → 102 ядра → 12 подов × 8 ядер = 96 ядер; RAM: 12 × 16 ГБ = 192 ГБ.

**media-service.** Ограничение по полосе: 410 Гбит/с / 25 Гбит (внутренний канал) ≈ 17 серверов → выбрано 24 пода × 8 ядер с запасом для CPU и буферизации.

**inbox-worker.** Каждое из 125 000 входящих сообщений/с порождает в среднем 2 записи в `inbox_events` → 250 000 строк/с. При батчинге (1 000 строк) и 10 000 батч-операций/ядро → 25 ядер; выбрано 80 ядер (10 подов × 8) для параллелизма по Kafka-партициям.

**Расчёт K8s-узлов (DC5).** Суммарный CPU request Go-сервисов:

```
32 + 96 + 192 + 32 + 24 + 80 + 24 + 24 = 504 ядра
```

С учётом системных подов и накладных расходов Kubernetes (+20%): ~605 ядер.
Конфигурация узла: 64 ядра, 512 ГБ RAM.
Количество: ⌈605 / 64⌉ = 10 → 12 узлов (включая 2 резервных для отказоустойчивости).

**PostgreSQL.** 3 группы шардов × 3 (1 мастер + 2 реплики) = 9 серверов; округлено до 12 с запасом под рост нагрузки.

**ScyllaDB.** RF = 3 (минимум 3 узла). В DC5 — 6 узлов (2 группы по 3 узла), в остальных ДЦ — 3 узла (RF = 3 обеспечивает кворум).

**Redis.** 3 мастер-шарда + 3 реплики = 6 узлов на каждый ДЦ (Redis Cluster). 512 ГБ RAM/сервер для хранения сессий и WS-маппинга с запасом.

**Kafka.** Минимум 3 брокера на каждый ДЦ (фактор репликации RF = 3).

## 11.2 Конфигурации физических серверов (все ДЦ)

| Название | Хостинг | Конфигурация | Ядра | Кол-во | Покупка | Аренда/мес |
| --- | --- | --- | --- | --- | --- | --- |
| **kubenode** | own | 2×AMD EPYC 7543 / 16×32 ГБ / 2×NVMe 4 ТБ / 2×25GbE | 64 | 32 | $18 000 | $300 |
| **lvs-l4** | own | 1×Intel Xeon E-2334 / 4×8 ГБ / 1×NVMe 512 ГБ / 2×100GbE | 8 | 32 | $4 000 | $67 |
| **nginx-l7** | own | 2×Intel Xeon Silver 4314 / 4×16 ГБ / 2×NVMe 512 ГБ / 2×100GbE | 16 | 108 | $5 500 | $92 |
| **db-postgres** | own | 2×AMD EPYC 7282 / 8×32 ГБ / 4×NVMe 4 ТБ / 2×25GbE | 32 | 30 | $15 000 | $250 |
| **db-scylla** | own | 2×AMD EPYC 7282 / 8×32 ГБ / 8×NVMe 8 ТБ / 2×25GbE | 32 | 18 | $22 000 | $367 |
| **db-redis** | own | 2×Intel Xeon Silver 4210R / 16×32 ГБ / 2×NVMe 512 ГБ / 2×25GbE | 20 | 30 | $12 000 | $200 |
| **kafka** | own | 2×Intel Xeon Silver 4210 / 4×32 ГБ / 4×NVMe 4 ТБ / 2×25GbE | 16 | 15 | $8 000 | $133 |
| **ceph-mon** | own | 1×Intel Xeon E-2334 / 2×16 ГБ / 2×NVMe 512 ГБ / 2×10GbE | 8 | 9 | $4 000 | $67 |
| **ceph-rgw** | own | 2×AMD EPYC 7282 / 4×32 ГБ / 2×NVMe 2 ТБ / 2×100GbE | 32 | 15 | $10 000 | $167 |
| **ceph-osd** | own | 1×Intel Xeon E-2334 / 4×16 ГБ / 12×HDD 18 ТБ / 2×25GbE | 8 | 55 | $18 000 | $300 |

55 OSD-узлов — начальная конфигурация запуска. Для полного объёма за год (~2 ЭБ, раздел 2.2): ~14 000 OSD-узлов; ёмкость наращивается поэтапно по мере роста хранилища.

**Распределение по дата-центрам:**

| Тип | DC5 (40%) | DC2 (17,5%) | DC4 (17,5%) | DC1 (12,5%) | DC3 (12,5%) | Итого |
| --- | --- | --- | --- | --- | --- | --- |
| kubenode | 12 | 6 | 6 | 4 | 4 | 32 |
| lvs-l4 | 10 | 6 | 6 | 5 | 5 | 32 |
| nginx-l7 | 40 | 20 | 20 | 14 | 14 | 108 |
| db-postgres | 12 | 6 | 6 | 3 | 3 | 30 |
| db-scylla | 6 | 3 | 3 | 3 | 3 | 18 |
| db-redis | 6 | 6 | 6 | 6 | 6 | 30 |
| kafka | 5 | 3 | 3 | 3 | 3 | 17 |

Ceph организован в **3 региональных кластера**: Азия (DC5), Европа (DC2 + DC4), Америка (DC1 + DC3).

| Кластер | ceph-osd | ceph-mon | ceph-rgw |
| --- | --- | --- | --- |
| Азия | 20 | 3 | 5 |
| Европа | 20 | 3 | 5 |
| Америка | 15 | 3 | 5 |
...
## 11.3 Аллокация ресурсов в Kubernetes (DC5)

| Сервис | CPU requests | CPU limits | RAM requests | RAM limits | Кол-во реплик |
| --- | --- | --- | --- | --- | --- |
| **message-service** | 8 | 8 | 4 ГБ | 8 ГБ | 4 |
| **sync-service** | 8 | 8 | 16 ГБ | 32 ГБ | 12 |
| **media-service** | 8 | 8 | 8 ГБ | 16 ГБ | 24 |
| **auth-service** | 8 | 8 | 2 ГБ | 4 ГБ | 4 |
| **chat-service** | 8 | 8 | 4 ГБ | 8 ГБ | 3 |
| **inbox-worker** | 8 | 8 | 4 ГБ | 8 ГБ | 10 |
| **archive-worker** | 8 | 8 | 2 ГБ | 4 ГБ | 3 |
| **notification-service** | 8 | 8 | 2 ГБ | 4 ГБ | 3 |

CPU limits равны CPU requests — throttling недопустим для Telegram. RAM limits удвоены относительно RAM requests как защита от аномальных всплесков без риска OOM при штатных пиках.
