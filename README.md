...
Алгоритм синхронизации состояния через Transactional Inbox (подкачка дельты)

## 7.1 Наименование принятого подхода
...
## 7.2 Применяемая подсистема

Подсистема синхронизации сообщений.

## 7.3 Постановка задачи

Обеспечение гарантированной доставки уведомлений о новых сообщениях на все устройства пользователя с соблюдением порядка и принципа «ровно один раз», независимо от стабильности сетевого соединения клиента.

## 7.4 Исходные данные, ограничения и требования

* Входные данные: поток событий из Kafka (partition key = `chat_id`).
* Ограничения:
    * Запрет на межшардовые транзакции.
    * Пиковая нагрузка — 697 916 RPS.
* Требования:
    * Минимизация трафика (передача только изменений — «дельты»).
    * Идемпотентность (защита от дублей).

## 7.5 Последовательность работы алгоритма
...
2. Сервер проверяет, существует ли `since_id` в `inbox_events`:
    * Если да (клиент офлайн <= 7 дней) — сервер возвращает дельту: все строки с `id > since_id` для данного `user_id`.
...
* **Kafka Topic:** партиционирование по `chat_id` для сохранения последовательности сообщений внутри чата.
* **ScyllaDB Table `messages`:** перманентное хранилище тел сообщений. Заполняется при помощи Archive Worker из Kafka. Partition key = `chat_id`, clustering key = `message_id` DESC.
* **PostgreSQL Table `inbox_events`:** персональная очередь событий для каждого пользователя (Transactional Inbox). Шардируется по `user_id`.
* **PostgreSQL Table `user_dialogues`:** список чатов пользователя с превью и счётчиком непрочитанных. Обновляется атомарно вместе с `inbox_events`.
* **Redis Set `ws:{user_id}`:** множество идентификаторов активных WebSocket-соединений пользователя. Позволяет доставить дельту на все устройства одновременно.

## 7.7 Альтернативные решения

1. Pull-модель: клиент раз в N секунд опрашивает основную таблицу `messages`.
    * *Минус:* огромная нагрузка на ScyllaDB при отсутствии новых сообщений. Неэффективное использование ресурсов и трафика.
2. Long Polling с Redis Sync: хранение `last_sync_message_id` в Redis, синхронизация через Long Polling.
    * *Минус:* состояние синхронизации не связано напрямую с данными. Нет возможности сразу выкачать дельту.
3. Transactional Outbox: API записывает данные и событие в PostgreSQL в одной транзакции, затем сервис-релей пересылает в Kafka.
    * *Минус:* добавляет промежуточную таблицу и сервис-релей; увеличивает латентность доставки. При шардированном PostgreSQL outbox привязан к шарду отправителя, что усложняет маршрутизацию.

## 7.8 Обоснование выбора

Выбранный вариант разделяет ответственность между компонентами:

* **Kafka** принимает события напрямую от API, обеспечивает буферизацию, надёжность и возможность подключения нескольких консьюмеров (Archive Worker + Inbox Worker).
* **Archive Worker** отвечает за запись тел сообщений в ScyllaDB.
* **Inbox Worker** переносит нагрузку по «раскладке» сообщений на фоновые воркеры, каждый из которых пишет только в один шард PostgreSQL.

Ключевые параметры:
* Шардирование `inbox_events` по `user_id`.
* Использование Inbox как «дельты» позволяет клиенту скачивать только минимально необходимый набор данных для актуализации состояния.
* Идемпотентность на уровне консьюмера (уникальный индекс `user_id, message_id`) защищает от дублирования при повторной обработке из Kafka.

# 8. Технологии

| Технология | Область применения | Мотивационная часть |
| :--- | :--- | :--- |
| **TypeScript + React** | Frontend | Статическая типизация TypeScript снижает количество ошибок в коде, а компонентная модель React с виртуальным DOM эффективно обновляет список чатов и историю переписки без перерисовки всего интерфейса |
...
| **Nginx** | L7-балансировщик, SSL Termination | Стандарт индустрии для реверс-проксирования и распределения запросов по микросервисам, а также поддержка Session Tickets для ускорения повторных TLS-соединений мобильных клиентов |
