...
| :--- | :--- |
| **Auth API** (JWT, Redis) | 8 000 RPS |
| **Core API** (Бизнес-логика, PG) | 3 000 RPS |
| **Message API** (Архив сообщений, Scylla) | 6 000 RPS |
| **Media API** (S3 Proxy) | 4 000 RPS |
| **Inbox worker** | 5 000 событий/с |
| **Archive worker** | 10 000 записей/с |
| **Push API** | 10 000 RPS |

### Сводная таблица ресурсов
| Сервис | Нагрузка (RPS) | Расчет | CPU (ядер) | RAM |
| :--- | :--- | :--- | :--- | :--- |
| **auth-service** | 258 334 | 258 334 / 8 000 ≈ 32 → 4 × 8 | 32 | 32 ГБ |
| **core-service** | 43 750 | 43 750 / 3 000 = 14.6 → 2 × 8 | 16 | 32 ГБ |
| **message-service** | 125 000 | 125 000 / 6 000 = 20.8 → 3 × 8 | 24 | 24 ГБ |
| **media-service** | 45 834 | 45 834 / 4 000 = 11.5 → 2 × 8 | 16 | 16 ГБ |
| **inbox-worker** | 250 000 | 250 000 / 5 000 = 50 → 10 × 5 | 50 | 10 ГБ |
| **archive-worker** | 125 000 | 125 000 / 10 000 = 12.5 → 3 × 5 | 15 | 6 ГБ |
| **notification-service** | 50 000 | 50 000 / 10 000 = 5 → 3 × 2 | 6 | 3 ГБ |
| **Итого** | | | **159** | **123 ГБ** |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Kubernetes Cluster** | CyberServe Xeon SP1-102G | Xeon Silver 4514Y / 128GB RAM / 2x1TB NVMe / 10Gbps | 16 | 33 | €5 490 | €181 170 | €3 020 | AX102: €122.30 × 33 = €4 035.90 |
...
| **ScyllaDB** | CyberServe Xeon SP1-102G | Xeon Silver 4514Y / 128GB RAM / 2x4TB NVMe / 10Gbps | 16 | 18 | €6 400 | €115 200 | €1 920 | AX162-S: €242.30 × 18 = €4 361.40 |
...
| **Итого** | | | | **323** | | **€1 765 490** | **€29 425** | **€72 562.90** |
...
| **media-service** | 2 | 8 | 8 ГБ | Проксирование в Ceph |
| **inbox-worker** | 10 | 5 | 1 ГБ | Kafka, inbox_events |
| **archive-worker** | 3 | 5 | 2 ГБ | Kafka, ScyllaDB архив |
| **notification-service** | 3 | 2 | 1 ГБ | push уведомления |
...
Примечание:
1. Для Kubernetes в DC5 требуется 159 CPU. С учетом 20% накладных расходов Kubernetes: 159 × 1,2 ≈ 191 CPU. Минимум: 191 / 16 ≈ 12 серверов. Для DC5 принято 13 серверов с запасом нагрузки; для остальных ДЦ количество снижено пропорционально доле трафика: 13 + 6 + 6 + 4 + 4 = 33 сервера.
