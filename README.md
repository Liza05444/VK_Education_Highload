...
Примечание:
1. Для Kubernetes в DC5 требуется 88 CPU. С учетом 20% накладных расходов Kubernetes: 88 × 1,2 ≈ 106 CPU. Минимум: 106 / 16 ≈ 7 серверов. Для DC5 принято 12 серверов с запасом нагрузки; для остальных ДЦ количество снижено пропорционально доле трафика: 12 + 6 + 6 + 4 + 4 = 32 сервера.
